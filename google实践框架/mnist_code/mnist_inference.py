...
# 在测试时会通过保存的模型加载这些变量的取值。而且更方便的是，因为可以在变量加载时将滑动平均变量重命名，
...
def inference(input_tensor, regularizer):
    ...
    with tf.variable_scope('layer1'):
        #这里通过tf.get_variable或tf.Variable没有本质区别，因为在训练或是测试中没有在同一个程序中
        #多次调用这个函数。如果在同一个程序中多次调用，在第一次调用之后需要将reuse参数设置为True
